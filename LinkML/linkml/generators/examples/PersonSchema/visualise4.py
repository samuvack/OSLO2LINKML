...
def run_command(input_yaml = "personinfo.yaml", output_yaml = "yuml.yuml"):
    # ... [Your existing run_command function] ...
    try:
...
def convert_yuml_to_uml(file_path = "yuml.yuml", output_path='test.svg'):
...
def main():
    input_yaml = "personinfo.yaml"
    output_yuml = "yuml.yuml"

    def process_yaml_file():
        logging.info("Processing YAML file...")
        run_command(input_yaml, output_yuml)
        convert_yuml_to_uml('yuml.yuml')
        # Reload the image in the viewer if needed
        if hasattr(app, 'viewer'):
            app.viewer.loadImage('test.svg')

    # Set up file monitoring
    event_handler = FileChangeHandler(input_yaml, process_yaml_file)
    observer = Observer()
    ...
    app.viewer.show()
    with stdout_redirect(Stream(newText=app.viewer.onUpdateText)):
        sys.exit(app.exec_())
